...
layui.define(['layer', 'form'], function (exports) {
    ...
    Edit.prototype.build = function (id, settings) {
        ...
        oScript.src="/static/layuiadmin/qiniu.min.js";
    ...
    var setIframe = function (editor, textArea, set) {
        ...
        , toolActive = function (iframeWin, editor, set) {
            var iframeDOM = iframeWin.document
                                    ...
                                    var subscription = observable.subscribe(
                                        {
                                            ...
                                            complete(res) {
                                                //insert(downloadurl + '?file=' + res.key)
                                                insertInline.call(iframeWin, 'img', {
                                                    src: downloadurl + '?file=' + res.key
                                                    ,alt:  name
                                                    ,style:'width:98%'
                                                }, range);
                                                layer.close(index);
                ...
                //附件
                , infiles: function (range) {
                    $('body').on('change','#infiles',function(){
                        var name = $($(this))[0].files[0].name;
                        var files = $($(this))[0].files[0];
                        var index = layer.load(1);
                        $.ajax({
        ...
        , tools = {
            ...
            ,
            image: '<i class="layui-icon layedit-tool-image" title="图片" layedit-event="image">&#xe64a;<input type="file" name="file" id="image" accept="image/gif,image/jpeg,image/jpg,image/png,image/svg"></i>'
            ,infiles: '<i class="layui-icon layedit-tool-image" title="附件" layedit-event="infiles">&#xe62f;<input type="file" name="infiles" id="infiles"></i>'
            ,
