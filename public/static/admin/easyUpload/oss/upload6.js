...
function img_reg(str){
    ...
    var sear7=new RegExp('.gif');
    var sear3=new RegExp('.JPG');
    var sear4=new RegExp('.PNG');
    var sear6=new RegExp('.JPEG');
    var sear8=new RegExp('.Gif');
    if(sear1.test(str) || sear2.test(str) || sear3.test(str) || sear4.test(str) || sear5.test(str) || sear6.test(str) || sear7.test(str) || sear8.test(str)){
        return true;   //是图片
    }
    return false; // 不是图片
}
...
function video_reg(str){
    var reg_arr = ['.MP4','.TS','.3GP','.MPG','.MPEG','.MPE','.DAT','.VOB','.ASF','.AVI','.WMV','.ASF','.WEBM','.RM','.RMVB','.MOV','.mp4','.ts','.3gp','.mpg','.mpeg','.mpe','.dat','.vob','.asf','.avi','.wmv','.asf','.webm','.rm','.rmvb','.mov']
    for (var i=0;i<reg_arr.length;i++)
    {
        var sear=new RegExp(reg_arr[i]);
        if(sear.test(str) ){
            return sear.test(str);   //是视频
        }
    }
    // reg_arr.forEach((item, index) => {
    //     var sear=new RegExp(item);
    //     if(sear.test(str) ){
    //         return true;   //是视频
    //     }
    // })
...
//验证是否是音频
function audio_reg(str){
    var reg_arr = ['.MP3','.WMA','.WAV','.AAC','.RA','.M4A','.FLAC','.APE','.AC3','.AMR','.CAF','.ACM','.mp3','.wma','.wav','.aac','.ra','.m4a','.flac','.ape','.ac3','.amr','.caf','.acm']
    for (var i=0;i<reg_arr.length;i++)
    {
        var sear=new RegExp(reg_arr[i]);
        if(sear.test(str) ){
...
var uploader = new plupload.Uploader({
    filters: {
        mime_types: [
            // {title: "video", extensions: "MP4,TS,3GP,MPG,MPEG,MPE,DAT,VOB,ASF,AVI,WMV,ASF,WEBM,RM,RMVB,MOV"},
             {title: "image", extensions: "jpg,png,jpeg,gif"},
            //{title: "audio", extensions: "MP3,WMA,WAV,AAC,RA,M4A,FLAC,APE,AC3,AMR,CAF,ACM"},
        ],
    ...
    init: {
        ...
        FilesAdded: function(up, files) {
            plupload.each(files, function(file) {
                ...
                }else if(audio_reg(file.name)) {
                    if(file.size > 128*1024*1024) {
                        //自动关闭
                        Dreamer.error('文件'+file.name+'超出音频限制大小',2000);
                        return true;
                    }
                } else {
        ...
        FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
                ...
                $.ajax({
                    type: "POST",
                    url: "/admin/bdsucai/stickers_ajax_uploadfile",
                    data: video_data,
                    dataType: "json",
        ...
        Error: function(up, err) {
            ...
            //layer.msg(err,{icon:2,time:1500,shade: 0.1});
            setTimeout(function(){
                location.reload();
            }, 1500);
