define(['jquery', 'bootstrap', 'backend', 'form', 'table'], function ($, undefined, Backend, Form, Table) {
    ...
    var Controller = {
        ...
        select: function () {
            ...
            var mimetype = Backend.api.query('mimetype');
            ...
            // 初始化表格
            table.bootstrapTable({
                url: $.fn.bootstrapTable.defaults.extend.index_url+'?mimetype='+mimetype,
                sortName: 'id',
                showToggle: false,
                showExport: false,
                ...
                columns: [
                    [
                        ...
                        // {field: 'title', title: '文件名', operate: 'like'},
                        {field: 'title', title: __('Filename'), formatter: Controller.api.formatter.url, operate: false},
                        {field: 'mimetype', title: __('Mimetype'),searchList: {"image":'图片',"audio":'音频','video':'视频'}, operate:'like'},
                       
                        {field: 'addtime', title: __('Createtime'), formatter: Table.api.formatter.datetime, operate: 'RANGE', addclass: 'datetimerange', sortable: true},
                        {
        ...
        api: {
            ...
            formatter: {
                thumb: function (value, row, index) {
                    if (row.url.indexOf("mp3") > -1) {
                        return '<a href="' + row.url + '" target="_blank"><img src="/storeadmin/ajax/icon?suffix=mp3"  style="max-height:90px;max-width:120px"></a>';
                    }else if (row.url.indexOf("mp4") > -1) {
                        return '<a href="' + row.url + '" target="_blank"><video src="' + row.url + '" alt="" style="max-height:90px;max-width:120px"></video></a>';
                    } else if (row.url.indexOf("jpg") > -1 || row.url.indexOf("png") > -1) {
                        return '<a href="' + row.url + '" target="_blank"><img src="' + row.url + '" alt="" style="max-height:90px;max-width:120px"></a>';
                    }
                },
                url: function (value, row, index) {
                    return '<a style="width:200px;margin:0 auto;text-align:center;overflow:hidden;white-space: nowrap;text-overflow: ellipsis;display:inline-block" href="' + row.url + '" target="_blank">' + row.title + '</a>';
                },
