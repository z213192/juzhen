...
require ROOT_PATH.'/extend/aliyun-oss/autoload.php';
...
class Ajax extends Backend
{
    ...
    public function upload()
    {
        ...
        if ($chunkid) {
            ...
            $file = $this->request->file('file');
            try {
                $ossClient = new OssClient(ALIYUN_KEYID, ALIYUN_SECRET, "https://oss-cn-shanghai.aliyuncs.com");
                $info = $file->getInfo();   
                // var_dump($info);
                $fileName = sha1(date('YmdHis', time()) . uniqid()) . strrchr($info['name'], '.');
                $result = $ossClient->uploadFile(ALIYUN_BUCKET, $fileName, $file->getInfo()['tmp_name']);
                // 入库file_media
                Db::name('file_media')->insert(['mimetype'=>$info['type'],'url'=>$result['info']['url'],'addtime'=>date('Y-m-d H:i:s'),'title'=>$info['name'],'file_size'=>$info['size'],'mediaId'=>'','requestId'=>'']);
                // echo '<pre>';
                // var_dump($result);exit;
                $this->success(__('Uploaded successful'), '', ['url' => $result['info']['url'], 'fullurl' => $result['info']['url']]);
